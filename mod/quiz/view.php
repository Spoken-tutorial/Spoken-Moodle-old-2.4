...
/* TestAttendance start */
#spoken db connection
global $USER;
include('spoken-config.php');

# user can attempt the test
$count = 0;
if(empty($viewobj->attempts)){

    $sql = "select id from  training_eventteststatus where mdlemail = ".$USER->email." and mdlcourse_id = ".$quiz->course." and status = 0";
    $result = mysql_query($sql);
    $count = mysql_num_rows($result);
}else{
    foreach($viewobj->attempts as $a){
        $sql = "select id from  training_eventteststatus where mdlemail = ".$USER->email." and mdlcourse_id = ".$quiz->course." and mdlquiz_id=".$a->quiz." and mdlattempt_id=".$a->id." and status <= 3";
        $result = mysql_query($sql);
        $count = mysql_num_rows($result);
    }
}
