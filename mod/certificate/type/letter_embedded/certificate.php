...
if($_GET['tc'] != ''){
	...
	$query = "SELECT tr.*, org.organiser_name, ac.institution_name, td.invigilator FROM test_requests tr, organiser org, academic_center ac,  test_details td WHERE tr.test_code='".$_GET['tc']."' and org.organiser_id=tr.organiser_id and tr.academic_code=ac.academic_code and td.test_code=tr.test_code and td.batch_no=".$_GET['batchs'];	
	$query_result = mysql_query($query);	
	$cert_info = mysql_fetch_array($query_result);
...
certificate_print_image($pdf, $certificate, CERT_IMAGE_SIGNATURE, $sealx, $sealy, '', '');
// Add text
/*
...
*/
$studentname = '<p>This is to certify that <b>'.fullname($USER).'</b> has successfully completed the '.$course->fullname.' organized at <b>'.$cert_info['institution_name'].'</b> by <b>'.$cert_info['organiser_name'].'</b> with course material provided by the Talk To A Teacher project at IIT Bombay.<br><br>Passing an online exam, conducted remotely from IIT Bombay, is a pre-requisite for completing this workshop. <b>'.$cert_info['invigilator'].'</b> at <b>'.$cert_info['institution_name'].'</b> invigilated this examination. This workshop is offered by the <b>Spoken Tutorial project, IIT Bombay, funded by National Mission on Education through ICT, MHRD, Govt of India.</b><br><br>'.date("d M Y", strtotime($cert_info['cfm_test_date'])).'</p>';
